...
GBFS_ROOT = {
    ...
    "paris": "https://velib-metropole-opendata.smoove.pro/gbfs/gbfs.json"  # 若需法语/多文件可扩展
}
...
def _put_json(obj:dict, key:str):
    ...
    with gzip.GzipFile(fileobj=gzbuf, mode="wb") as gz:
        gz.write(body)
    s3.put_object(Bucket=BUCKET, Key=key, Body=gzbuf.getvalue(),
...
def handler(event, context):
    ...
    if _exists(status_key) and _exists(info_key):
        return {"skipped": True, "prefix": base}

    _put_json(status, status_key)
    _put_json(info, info_key)
    # 写一个小清单 manifest
    _put_json({"city": CITY, "source_ts": status["last_updated"], "ingested_utc": int(time.time())},
              base + "_manifest.json.gz")

    return {"ok": True, "prefix": base}
