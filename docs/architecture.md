...
- station staging owns the physical `snapshot_bucket_at` business grain
- `station_id` alone is not treated as a globally safe warehouse key
- `dim_station` should use `city + station_id` as its durable business key, exposed as `station_key`
- `dim_station` should be SCD2 so station attributes can be joined point-in-time by version
- `fct_station_status` should stay lean and use `station_version_key` for the station SCD2 join
